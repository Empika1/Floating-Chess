...
public class GameScreen extends JPanel {
    ...
    public void startGame() { // starts the game loop. done in a thread so other stuff like event handling
                              // still happens
        Thread gameThread = new Thread(() -> {
            while (!isGameOver) { // if the game is stil going, do all this stuff
                mousePosBoard = board.mousePos; // set all the mouse stuff to what it should be. this is done in advance
                                                // to avoid multithreading shenanigans
    ...
    public void movePieces() { // alright, get ready
        board.turn = board.turnNumber % 2 == 0 ? ChessColor.BLACK : ChessColor.WHITE; // set the turn to what it should
                                                                                      // be

        Vector2I boardBuffer = new Vector2I(-50, -50); // the buffer of pixels around the board where the mouse will be
                                                       // considered to
        if ((!Geometry.isPointInRect(boardBuffer, Board.boardSizeI.subtract(boardBuffer), mousePosBoard))
                && board.heldPiece != null) { // if the mouse is not in the board, and a piece was being held, stop
                                              // holding it
        ...
        } else if (mouseLeftPressedBoard && board.heldPiece == null) { // else if the left mouse button is pressed and
                                                                       // there is not a piece being held yet
            if (board.turn == ChessColor.WHITE) { // loop through all the pieces on the board of the right color, find
                                                  // which one is being held, and make it the held piece
        ...
        if (board.heldPiece != null) { // if there is a piece being held
            ...
            Rook leftRook = null;
            if (board.heldPiece.getPieceType() == PieceType.KING && !board.heldPiece.getHasMoved()) { // if the king is
                                                                                                      // being held, and
                                                                                                      // it hasn't moved
                                                                                                      // in the game yet
                ...
                for (Piece p : sameColorPieces) { // loop through the pieces, and if there is a rook on the right side
                                                  // of the board, make it the rightrook
                ...
                if (rightRook != null) // if there is a right rook, then set its visible position to its actual
                                       // position. this is so it doesnt get stuck in its castling position if you
                                       // abort a castle
                    rightRook.setVisiblePos(rightRook.getTruePos().copy());
                if (leftRook != null)
                    leftRook.setVisiblePos(leftRook.getTruePos().copy()); // dito for leftrook
                if (board.heldPiece.getVisiblePos().x == (int) (Board.boardSizeI.x * 6.5 / 8)) { // if the king is in
                                                                                                 // its right castling
                                                                                                 // position
                    rightRook.setVisiblePos(
                            new Vector2I((int) (Board.boardSizeI.x * 5.5 / 8), rightRook.getTruePos().y)); // set the
                                                                                                           // rightrook
                                                                                                           // to its
                                                                                                           // corresponding
                                                                                                           // castling
                                                                                                           // position
                } else if (board.heldPiece.getVisiblePos().x == (int) (Board.boardSizeI.x * 2.5 / 8)) { // if the king
                                                                                                        // is in its
                                                                                                        // left castline
                                                                                                        // position
                    leftRook.setVisiblePos(
                            new Vector2I((int) (Board.boardSizeI.x * 3.5 / 8), leftRook.getTruePos().y)); // set the
                                                                                                          // leftrook to
                                                                                                          // its
                                                                                                          // corresponding
                                                                                                          // castling
            ...
            if (!mouseLeftPressedBoard) { // if the left button is released (the move is made)
                board.heldPiece.setTruePos(board.heldPiece.getVisiblePos(), true); // move the heldpiece to its proper
                                                                                   // position
                if (rightRook != null)
                    rightRook.setTruePos(rightRook.getVisiblePos(), true); // move the rooks to their proper positions
                ...
                if (board.heldPiece.getPieceType() == PieceType.PAWN) { // if the piece that just moved was a pawn, and
                                                                        // it got to the pawn promotion area, show the
                                                                        // promotion dialog
                    if (board.heldPiece.getColor() == ChessColor.WHITE) {
                        if (board.heldPiece.getTruePos().y <= Board.boardSizeI.y * 0.5 / 8 + 2) // the +2 and -2 are
                                                                                                // just to account for
                                                                                                // if the pawn is very
                                                                                                // close but not quite
                                                                                                // at the end of the
                ...
                backButton.setEnabled(true); // enable the back button (this is disabled at the start so you cant go
                                             // back from the starting position and break stuff)
            }
        }
    }
    ...
    void showPiecePromotionDialog(Pawn p) { // shows the dialog for promoting a pawn
        ...
        String n = null;
        while (n == null) //makes it so if the cancel button is pressed, it shows the dialog again (there is no way to remove the cancel button)
            n = (String) JOptionPane.showInputDialog(null,
    ...
    void showGameOverDialog() {
        ...
        String winString = lossState == LossState.KINGCAPTURED
                ...
                : wonColor + " wins, as " + lostColor + " has run out of time!"; // the string to be displayed on the
                                                                                 // dialog
    ...
    void backOneMove() {
        if (board.heldPiece == null) { // just so you can't do any shenanigans with going back a move while holding a
                                       // piece somehow
            Move lastMove = gameReplay.moves.get(gameReplay.moves.size() - 2).copy(); // get the move just before the
                                                                                      // move that was just made
            board.whitePieces = lastMove.whitePieces(); // set all the settings of the board to what they were at that
                                                        // move
